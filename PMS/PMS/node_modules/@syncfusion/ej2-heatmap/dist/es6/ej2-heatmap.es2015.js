import { ChildProperty, Property, Complex, Collection, isNullOrUndefined, extend, createElement, compile, remove, Browser, merge, select, print, Component, Internationalization, EventHandler, Touch, Event, NotifyPropertyChanges } from '@syncfusion/ej2-base';
...
class AxisHelper {
    ...
    drawXAxisTitle(axis, parent, rect) {
        ...
        if (axis.title.text) {
            ...
            const elementSize = measureText(title.text, title.textStyle);
            ...
            const options = new TextOption(heatMap.element.id + '_XAxisTitle', new TextBasic(rect.x + titlePositionX(rect.width, 0, 0, title.textStyle), y + padding, anchor, title.text), title.textStyle, title.textStyle.color || heatMap.themeStyle.axisTitle);
    ...
    drawYAxisTitle(axis, parent, rect) {
        if (axis.title.text) {
            ...
            const options = new TextOption(heatMap.element.id + '_YAxisTitle', new TextBasic(x, y - this.padding, anchor, title.text, labelRotation, 'rotate(' + labelRotation + ',' + (x) + ',' + (y) + ')', 'auto'), title.textStyle, title.textStyle.color || heatMap.themeStyle.axisTitle);
            if (!this.heatMap.enableCanvasRendering) {
                this.drawSvgCanvas.createText(options, parent, title.text);
            }
            else {
    ...
    drawXAxisLabels(axis, parent, rect) {
        ...
        for (let i = 0, len = labels.length; i < len; i++) {
            ...
            let label = (axis.textStyle.textOverflow !== 'Wrap' && !(isLineBreak) && axis.textStyle.textOverflow !== 'Trim' && axis.labelIntersectAction === 'Trim' && axis.isIntersect) ? axis.valueType !== 'DateTime' ||
            ...
            const elementSize = measureText(wrappedLabel, axis.textStyle);
            let transform;
        ...
        axis.multiLevelLabels.map((multiLevel, level) => {
            ...
            multiLevel.categories.map((categoryLabel, i) => {
                if (!isNullOrUndefined(categoryLabel.start) && !isNullOrUndefined(categoryLabel.end)) {
                    ...
                    const textBasic = new TextBasic(x, y, anchor, categoryLabel.text, 0, 'translate(0,0)');
    ...
    renderYAxisMultiLevelLabels(axis, parent) {
        ...
        axis.multiLevelLabels.map((multiLevel, level) => {
            ...
            multiLevel.categories.map((categoryLabel, i) => {
                ...
                const textBasic = new TextBasic(x, y, 'start', categoryLabel.text, 0, 'translate(0,0)');
        ...
        for (let x = 0; x < (heatMap.xLength * heatMap.yLength); x++) {
            ...
            if (cellSetting.tileType === 'Rect') { // Rectangle/Tile Series
                ...
                if (cellSetting.bubbleType === 'Color') { // Bubble by same size and different color Series
                    ...
                    const tempCircleRadius = this.getRadiusBypercentage(parseFloat(this.text.toString()), heatMap.dataSourceMinValue, heatMap.dataSourceMaxValue, circleRadius);
...
class Legend {
    ...
    renderTitle(rect) {
        ...
        if (heatMap.legendSettings.title.text) {
            ...
            const titleSize = measureText(title.text, title.textStyle);
    ...
    getMaxLabelSize() {
        ...
        if (!heatMap.legendSettings.showLabel || (heatMap.horizontalGradient && heatMap.paletteSettings.type === 'Fixed' &&
            ...
            for (let i = 0; i < heatMap.legendColorCollection.length; i++) {
                const size = measureText(this.labelCollections[i], heatMap.legendSettings.textStyle);
                labelSize.width = (labelSize.width > size.width) ? labelSize.width : size.width;
                labelSize.height = (labelSize.height > size.height) ? labelSize.height : size.height;
            }
            return labelSize;
    ...
    renderPagingElements() {
        ...
        if (this.numberOfPages > 1) {
            ...
            const leftArrowRect = new RectOption(heatMap.element.id + '_leftArrow', 'none', { color: 'transparent', width: 0 }, 1, new Rect(leftX, rightArrowY - iconSize / 2, iconSize, iconSize));
...
let HeatMap = class HeatMap extends Component {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    triggerClickEvent(e, isDoubleClick) {
        let pageX;
        let pageY;
        let touchArg;
        const elementRect = this.element.getBoundingClientRect();
    ...
    heatMapMouseClick(e) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const position = this.triggerClickEvent(e, false);
    ...
    createMultiCellDiv(onLoad) {
        if (onLoad) {
            ...
            const element = document.getElementById(this.element.id + '_Multi_CellSelection_Canvas');
            const secondaryCanvas = this.secondaryCanvasRenderer.createCanvas({
                width: this.availableSize.width,
